...
public class ObjectMoveOnTriggerDelay : MonoBehaviour
{
    public enum TouchAction { MoveObject, ToggleActiveState, ScaleObject }
    public enum ActiveTouchMode { SetState, FlipState }
    ...
    [Tooltip("How much to ADD to the object's original scale.")]
    ...
    private void Awake()
    {
        triggerSoundSource = GetComponent<AudioSource>() ?? gameObject.AddComponent<AudioSource>();
    ...
    private void Update()
    {
        if (resolvedMoveTarget == null || !initialized) return;

        if (action == TouchAction.MoveObject && isMoving)
        {
            resolvedMoveTarget.position = Vector3.MoveTowards(
                resolvedMoveTarget.position, targetPosition, moveSpeed * Time.deltaTime);
            ...
            if ((resolvedMoveTarget.position - targetPosition).sqrMagnitude <= 0.0001f)
            {
                ...
                isMoving = false;
                if (moveOnce) hasMoved = true;
            }
        }
        else if (action == TouchAction.ScaleObject && isScaling)
        {
            resolvedMoveTarget.localScale = Vector3.MoveTowards(
                resolvedMoveTarget.localScale, calculatedTargetScale, scaleRate * Time.deltaTime);
